...
ただし現在は、GiNZA が返す entity のうち、次のような noisy label は `has_named_entity` や保存強度 bonus には使わないようにしています。
...
つまり `entities` 自体は保存するが、**記憶強度に効かせる named entity はより保守的に選ぶ** という方針です。
...
`ほしい`, `欲しい` のような明示的な希望表現、または lemma として `望む` が取れたときに付きます。  
`たい` は日本語の願望助動詞ですが、現在は **そのまま `desire` に落とさず**、周辺文脈で次のように切り分けています。

- plain desire:
  - 単純な「〜したい」という希望
  - `desire` を付与
- execution intent:
  - `入れておきたい`, `分けたい`, `整理したい` のような実行意図
  - `task` を付与
- habit / policy intent:
  - `形にしたい`, `流れにしたい`, `方法にしたい` のような継続方針・習慣化
  - `task` と `reflection` を付与

つまり、`たい` を一律に「欲求」と扱うのではなく、**願望・実行意図・習慣化意図を分ける** 方針です。

### `worry`

`不安`, `心配`, `困る`, `怖い`, `落ち着かない` のような表現、または関連 lemma として `悩む` が取れたときに付きます。  
継続的な悩みや気がかりの候補として扱います。

### `reflection`

`思う`, `感じる`, `気がする`, `おかしくないか`, `違和感` のような表現、または活用後でも lemma として `思う`, `感じる` が取れたときに付きます。  
...
`決めてほしい`, `選んでほしい`, `どれ`, `迷う` のような表現、または lemma として `決める`, `迷う` が取れたときに付きます。  
...
`不安`, `怖い`, `心配` のような表現、または関連 lemma で判定します。  
気がかりや警戒のある発話として扱います。

### `curiosity`

`知りたい`, `興味`, `試したい` のような表現、または関連 lemma で判定します。  
探索意欲や関心の高さとして扱います。
...
ただし `忘れないように` のような目的表現や、`じゃなくて` のような対比表現は、以前より強い negation として扱いにくくしています。  
例えば「不安はない」は、単語だけ見れば `anxiety` に見えますが、今は `neutral` に寄せるようにしています。

現在の negation 処理は、単純な文字列一致ではなく **GiNZA 上の negation cue ベース** です。  
具体的には、`ない / ぬ / ず` を Japanese UD の `AUX` / 述語的トークンとして見つけた上で、その cue が本当に記憶強度を弱める種類かを判定しています。

そのうえで、次のような表現は `has_negation` の付与対象から外しています。
...
つまり、今の `has_negation` は「文中に否定語がある」ではなく、**negation cue が実際に感情や記憶タイプを弱めるべきか** をかなり保守的に見ています。  
これは、日本語 negation を cue / scope / focus で扱う既存研究の考え方を、MVP 向けに軽く取り入れたものです。
...
- topic が4件以上ある
- または topic が3件以上かつ `entities` がある
- `entities` がある
- `TECHNICAL_MARKERS` がある
...
- `has_sensitive_topic` があると `+0.08`
- `has_decision_support` があると `+0.06`
- `has_future_reference` があると `+0.05`
- `has_named_entity` があると `+0.02`
- `has_reflection` があると `+0.05`
- `has_worry` があると `+0.05`
- `has_relationship` があると `+0.06`
- `is_short_ack` があると `-0.20`
- `is_low_information` があると `-0.10`
- `reason_codes` が `["is_context_only"]` だけなら `-0.08`

重みの意図は、感情・自己理解・関係性を主軸にしつつ、実用性や将来想起も副次的に残すことです。  
以前の式は 300 件の評価サンプルで `0.30` 台へ極端に詰まり、`high` と `critical` がほぼ出ませんでした。  
そのため、今の式では感情価値・関係性・内省・不安の寄与を少し強め、相槌や薄い記憶だけをはっきり落とす方向へ寄せています。
...
この閾値は、300 件の scratch 評価で実際の分布を見たうえで切り直しています。  
以前の `critical >= 0.80` と `high >= 0.55` は、今のルールベース解析ではほぼ到達不能でした。  
現在は、`critical` を「本当に強い上位少数」、`high` を「かなり強く残したい記憶」として現実的に出る水準へ寄せています。
...
「ほしい」「欲しい」や、plain desire と判定された `たい` 系の希望が含まれていることを示します。  
将来の行動や選択支援につながるため、保存価値があるとみなします。

### `has_execution_intent`

`〜したい` が、単なる願望ではなく「今後やる予定の操作・整理・更新」だと判定されたことを示します。  
実装上は `task` 側へ寄せ、保存価値は `has_task_signal` と合わせて評価します。

### `has_habit_intent`

`〜したい` が、継続運用・習慣化・方針化の意図だと判定されたことを示します。  
単なる実行予定より長期記憶寄りなので、`task` に加えて `reflection` も付けることがあります。
